...
```{r Tonight's Game Data}
gameData <- as.data.frame(jsonlite::fromJSON(paste(api,"/api/v1/schedule", sep = "" )))
#gameData <- data.frame(matrix(unlist(gameData), nrow = nrow(gameData), byrow = F))
gameDataNames <- names((gameData))
names(gameData) <- gameDataNames
 
gD2 <- as.data.frame(gameData$dates.games[[1]])
# gNames <- names(gD2)
# gD2 <- unlist(gD2, recursive = TRUE)
# gD2 <- as.data.frame(gD2, )
# 
# gNames <- names(gD2)
# gD2 <- as.data.frame(unlist(gD2), ncol = ncol(gD2))
# 
# gameData2 <- data.frame(matrix(unlist(gameData$dates.games), nrow = nrow(gameData$dates.games[[1]]), byrow = F))
# gameDataNames <- names(unlist(gameData2$dates.games[[1]]))
# names(gameData2) <- gameDataNames
```
 
```{r Season's Game Data}
 
completedgames <- list()
 
for(i in 1:nrow(teamData)){
  completedGameData <- as.data.frame(jsonlite::fromJSON(paste(api,"/api/v1/schedule?teamId=", teamData$teams.id[i], "&startDate=2017-08-01&endDate=", as.character(format(Sys.time(), "%Y-%m-%d")), sep = "")))
...
coordinatedPlays <- allGamePlays[!is.na(allGamePlays$x) & !is.na(allGamePlays$y),]

shots <- coordinatedPlays[coordinatedPlays$event == "Shot",]
goals <- coordinatedPlays[coordinatedPlays$event == "Goal",]
hits <- coordinatedPlays[coordinatedPlays$event == "Hit",]

ggplot2::ggplot(data = coordinatedPlays, mapping = aes(x=x, y=y, col = event)) +
  geom_point() +
  geom_point(data = shots, mapping = aes(x=x, y=y)) +
  geom_point(data = goals, mapping = aes(x=x, y=y))

ggplot2::ggplot(data = shots, mapping = aes(x=x, y=y, col = event)) +
  geom_point()

ggplot2::ggplot(data = hits, mapping = aes(x=x, y=y, col = event)) +
  # stat_density_2d(aes(fill = ..density..), geom = 'tile', contour = F)+
  # scale_fill_gradient(low = "#006400", high = "#CC0000") +
  stat_density2d(colour = "black", alpha = 0.6) +
  geom_point(alpha = 0.1, pch = 20, size = 2.8) + 
  geom_vline(aes(xintercept = 0), linetype = 'dashed', colour = "red", size = 1.5) +
  geom_vline(aes(xintercept = 25), colour = "blue", size = 1.5) +
  geom_vline(aes(xintercept = -25), colour = "blue", size = 1.5)

ggplot2::ggplot(data = goals, mapping = aes(x=x, y=y)) +
  stat_density_2d(aes(fill = ..density..), geom = 'tile', contour = F)+
  scale_fill_gradient(low = "#006400", high = "#CC0000") +
  stat_density2d(colour = "white", alpha = 0.6) +
  geom_point(alpha = 0.1, pch = 20, size = 2.8) + 
  geom_vline(aes(xintercept = 0), linetype = 'dashed', colour = "red", size = 1.5) +
  geom_vline(aes(xintercept = 25), colour = "blue", size = 1.5) +
  geom_vline(aes(xintercept = -25), colour = "blue", size = 1.5)
