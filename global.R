...
GoalieData <- readRDS(file = "GoalieData.RDS")
...
# source('GetGoalieData.R')    ## Collect information about Goalies (Shots and Goals against)

TeamLookup <- data.frame(triCode = c("MIN", "BOS", "SJS", "TOR", "FLA", "NYI", "ANA", "NSH", "VAN", "OTT", "EDM", "DET", "LAK", "NYR", "WSH",
                                     "CAR", "WPG", "VGK", "DAL", "STL", "BUF", "MTL", "CHI", "CGY", "CBJ", "PHI", "TBL", "NJD", "ARI", "PIT",
                                     "COL"), 
                         FullName = c("Minnesota Wild","Boston Bruins","San Jose Sharks","Toronto Maple Leafs","Florida Panthers","New York Islanders","Anaheim Ducks","Nashville Predators","Vancouver Canucks","Ottawa Senators","Edmonton Oilers", "Detroit Red Wings","Los Angeles Kings","New York Rangers","Washington Capitals","Carolina Hurricanes","Winnipeg Jets","Vegas Golden Knights","Dallas Stars","St Louis Blues","Buffalo Sabres","Montreal Canadiens","Chicago Blackhawks","Calgary Flames","Columbus Blue Jackets","Philadelphia Flyers","Tampa Bay Lightning","New Jersey Devils","Arizona Coyotes","Pittsburgh Penguins","Colorado Avalanche"))
...
## GOAL GENERATING SHOTS
count(SGGs$ZoneName)[order(count(SGGs$ZoneName)$freq),]
...
Zone_Summary <- function(PlaysetNoun = "League", Identifier, Statistic.To.Plot = "None", Logo, StatCompareTo = "None"){
   if(PlaysetNoun == "League" | is.null(PlaysetNoun)){
      Playset <- AllPlays
   } else if(PlaysetNoun == "Team"){
      Playset <- AllPlays[AllPlays$triCode == Identifier,]
   } else if(PlaysetNoun == "Skater"){
   ...
   } else if(PlaysetNoun == "Goalie"){
      Playset <- GoalieData[GoalieData$Goalie == Identifier,]
   }
   ...
   ZoneSummary <- merge(ShotZoneSummary,GoalZoneSummary, by = "Zone", all = T)
   ...
   ## Comparative Plots to Position (Inactive) #####
   # if(PlaysetNoun == "League"){
...
ShootingStatisticsPlots <- function(zoneShooting,zoneShootingLabels,Statistic.To.Plot,Logo){
   if(Statistic.To.Plot == "Shooting Percentage"){
      ...
      SSPlot <- ggplot(zoneShooting, aes(x = x, y = y)) +
         ...
         One.Side.Arena$layers +
         geom_label(data = zoneShootingLabels, aes(label = zoneShootingLabels$Label, x = X, y = Y, 
                                                   fill = `Shooting Percentage`), size = 3, colour = "black") +
         scale_fill_gradient2(midpoint = 8, low = "#FF0000", mid = "#FFFFFF", high = "#00FF00") +
         # scale_fill_distiller(type = "div", palette = 7, direction = 1) +
         theme_bw() + theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
                            axis.title = element_blank(), axis.ticks = element_blank(), panel.border = element_blank(), 
   ...
   if(Statistic.To.Plot == "Shots"){
      ...
      SSPlot <- ggplot(zoneShooting, aes(x = x, y = y)) +
         ...
         geom_label(data = zoneShootingLabels, aes(label = zoneShootingLabels$Shots, x = X, y = Y, 
                                                   fill = zoneShootingLabels$Shots), size = 3, colour = "black") +
   ...
   if(Statistic.To.Plot == "Goals"){
      ...
      SSPlot <- ggplot(zoneShooting, aes(x = x, y = y)) +
         ...
         geom_label(data = zoneShootingLabels, aes(label = zoneShootingLabels$Goals, x = X, y = Y, 
                                                   fill = zoneShootingLabels$Goals), size = 3, colour = "black") +
         scale_fill_distiller(type = "seq", palette = 10, direction = 1) +
         theme_bw() + theme(axis.text.x = element_blank(), axis.text.y = element_blank(),
                            axis.title = element_blank(), axis.ticks = element_blank(), 
                            panel.border = element_blank(), panel.grid.major = element_blank(),
...
HeatMapPlot <- function(PlaysetNoun, Identifier, Statistic.To.Plot, Sides){
   ...
   if(PlaysetNoun == "Skater"){
      Plays <<- AllPlays[AllPlays$player.fullName1 == Identifier,]
      try(currentTeamLogo <- rasterGrob(readPNG(paste0("www/",unique(Plays$triCode),".png")), interpolate = TRUE))
   }
   if(PlaysetNoun == "Goalie"){
      Plays <<- GoalieData[GoalieData$Goalie == Identifier,]
      try(currentTeamLogo <- rasterGrob(readPNG(paste0("www/NHL.png")), interpolate = TRUE))
   }
   
   if(Statistic.To.Plot == "Goals") Events <- Plays[Plays$event == "Goal",]
   if(Statistic.To.Plot == "Shots") Events <- Plays[Plays$event == "Shot",]

   if(Sides == "Two") DensityRinkPlot(Events, currentTeamLogo)
   if(Sides == "One") OneSideDensityRinkPlot(Events, currentTeamLogo)

}

ShootingZonePlot <- function(PlaysetNoun = "Team", Identifier = "CGY", Statistic.To.Plot = "Goals", StatCompareTo = "None"){
   if(PlaysetNoun == "Team"){
      currentTeamLogo <- rasterGrob(readPNG(paste0("www/",Identifier,".png")), interpolate = TRUE)
   }
   if(PlaysetNoun == "All") {
...
PlotCaller <- function(PlaysetNoun = "Team", Identifier = "CGY", ID2 = "EDM", Statistic.To.Plot = "Goals", input, output){
## The PlotCaller function is the master caller for all the plots.  Generally, when any variable is changed (Player, Team, Statistic, etc.) every plot needs to be redrawn.  Rather than including in the ui.R file a call to every different plot function which would make it pretty hard to read, instead they pass the arguments directly into the PlotCaller which then passes them on to the necessary plotting functions.
   ...
   output$PlayerStatsTable <- DT::renderDataTable(PlayerStats[,c(-2,-3)], style = "bootstrap")
   ...
   ## If the playset is team and the statistic shots, or the playset is league
      output$OneSideHeatMap <- shiny::renderPlot(HeatMapPlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
                                                             Statistic.To.Plot = DensityStatistic, Sides = "One"), width = 700, height = 600)
      # output$GOneSideHeatMap <- shiny::renderPlot(HeatMapPlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
      #                                                         Statistic.To.Plot = DensityStatistic, Sides = "One"), width = 700, height = 600)
      
      # output$TwoSideHeatmap <- shiny::renderPlot(HeatMapPlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
      #                                                        Statistic.To.Plot = DensityStatistic, Sides = "Two"), width = 1400, height = 600)

   output$OneSideZonemap <- shiny::renderPlot(ShootingZonePlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
                                                               Statistic.To.Plot), width = 700, height = 600)
   output$ZoneCompareLeague <- shiny::renderPlot(ShootingZonePlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
                                                                  Statistic.To.Plot = Statistic.To.Plot, 
                                                                  StatCompareTo = "League"), width = 700, height = 600)
   output$ZoneComparePosition <- shiny::renderPlot(ShootingZonePlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier, 
                                                                    Statistic.To.Plot = Statistic.To.Plot, 
                                                                    StatCompareTo = "Position"), width = 700, height = 600)
   
   output$SbSPlot1 <- shiny::renderPlot(ShootingZonePlot(PlaysetNoun = PlaysetNoun, Identifier = Identifier,
                                                            Statistic.To.Plot = Statistic.To.Plot), width = 700, height = 600)
   output$SbSPlot2 <- shiny::renderPlot(ShootingZonePlot(PlaysetNoun = PlaysetNoun, Identifier = ID2,
                                                            Statistic.To.Plot = Statistic.To.Plot), width = 700, height = 600)
